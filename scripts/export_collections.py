...
def export_collection(collection_pid_path):
    ...
    if collection_child_pids["collections"]:
        write_file_to_collection_directory(
            collection_pid_path,
            "{}.child-collections.pids".format(collection_file_prefix),
        ...
        for child_collection in collection_child_pids["collections"]:
            collections_to_process.append(
                "{}/{}".format(collection_pid_path, child_collection)
            )
...
def write_collections_with_noncollection_children_list():
    if collections_with_noncollection_children:
        ...
        log(
            "The following collections with noncollection children were detected and saved to root/collections_with_noncollection_children.txt:\n{}".format(
                formatted_data
            )
        )
        file = open(
...
log(
    "Export took {} seconds to process {} collections: \n{}".format(
        totaltime, len(collections_processed), "\n".join(collections_processed)
    )
)
